...
**Payments are idempotent and server-verified.** The client never determines whether a payment succeeded — a `PaymentGateway` interface abstracts the provider (a `MockGateway` for local demo, a `RazorpayGateway` doing real HMAC signature verification for production), and a unique constraint on the gateway's payment ID makes webhook retries safe. Invoice generation is similarly idempotent: re-running it for a month only touches invoices still in `pending` status.
